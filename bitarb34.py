...
class Exchange(Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            data = urlopen(self.url).readall().decode('utf-8')

            self.ask = int(data.split(self.sask + '\":')[-1].split(',')[0].split('}')[0].split('.')[0])
            self.bid = int(data.split(self.sbid + '\":')[-1].split(',')[0].split('}')[0].split('.')[0])
        
            up = int(data.split(self.last + '\":')[-1].split(',')[0].split('}')[0].split('.')[0])
...
class USDExchange(Exchange):
    ...
    def run(self):
        ...
        while True:
            ...
            if self.name == 'BTC-e':
                self.ask = float(data.split(self.sask + '\":')[-1].split(',\"')[0]) * OANDA.PRICE['USD_JPY']
                self.bid = float(data.split(self.sbid + '\":')[-1].split(',\"')[0]) * OANDA.PRICE['USD_JPY']
        
                up = float(data.split(self.last + '\":')[-1].split(',\"')[0]) * OANDA.PRICE['USD_JPY']
